...
class AbstractSolver(ABC):
    ...
    def calc_reward(self, state):
        # Create a new reward function for specific domains, like CartPole
        try:
            domain = self.env.unwrapped.spec.id
        except AttributeError:
            # Handle custom environments like SingleSEAEnv that don't have 'unwrapped.spec.id'
            if hasattr(self.env, 'name'):
                domain = self.env.name
            else:
                domain = None

        # Specific reward calculation for "CartPole-v1"
        ...
        # Default: return 0 if no specific reward function is required
        return 0
